...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                    ...
                    "[Q]Penjualan Harian Toko","[A]Deteksi Dini Corona","Surat Keterangan Covid"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "surat_keterangan_covid='"+tbUser.getValueAt(i,622).toString()+"'");
            }            
            tampil();
            emptTeks();
        }
                        ...
                        if (reply == JOptionPane.YES_OPTION) {
                            ...
                            if(i!= -1){
                                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                                    ...
                                    "surat_keterangan_covic='"+tbUser.getValueAt(barisdicopy,622).toString()+"'");
                            }    
                            userdicopy="";
                            copyhakakses="";
                            barisdicopy=-1;
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement("select AES_DECRYPT(id_user,'nur'),AES_DECRYPT(password,'windi'),"+
                        ...
                        "toko_bayar_piutang,toko_piutang_harian,toko_penjualan_harian,deteksi_corona,surat_kontrol,surat_keterangan_covid from user order by AES_DECRYPT(id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";

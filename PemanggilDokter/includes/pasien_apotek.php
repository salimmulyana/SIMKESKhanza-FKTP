...
<body >
		<audio id="suarabel" src="suara/airport.wav"></audio>
		<audio id="suarabeel" src="suara/airport2.wav"></audio>
		<audio id="pasienlama" src="suara/apotek.wav"></audio>
		<audio id="suarabelnomorurut" src="suara/no_urut.wav"  ></audio>
		<audio id="b" src="suara/c.wav"  ></audio>
		<audio id="suarabelsuarabelloket" src="suara/loketapotek.wav"  ></audio>
		...
		<?php
		$get_antrian = fetch_assoc(query("SELECT antrian FROM antriapotek"));
		$tcounter = $get_antrian['antrian'];
		$_tcounter = $tcounter + 1;
		if(isset($_GET['loket'])) {
			query("UPDATE antrian_loket SET end_time = CURRENT_TIME() WHERE type = 'Apotek' AND noantrian = '{$tcounter}'");
			query("UPDATE antriapotek SET loket = '{$_GET['loket']}', antrian = '{$_tcounter}'");
		}
		...
		<div align="center" style="font-size: 64px;color:white; text-shadow: 2px 2px 4px #000000;margin: 40px;">Sistem Antrian Apotek <form method="POST" action=""><input type="submit" class="btn btn-lg" value="Reset"></form></div>
									    ...
									    <div class="row justify-content-center">
												<div class="card-deck">
											      ...
											      <div class="card-header" style="font-size:41px;">Apotek</div>
												    <div class="card-body">
															<h5 class="card-title" style="font-size:100px;">C<?php echo $antrian; ?></h5>
												    </div>
												    <div class="card-footer p-0">
															<div class="btn-group btn-group-justified">
															  <a href="#" class="btn btn-primary" style="font-size:41px;"><?php $q = fetch_assoc(query("SELECT MAX(noantrian) as q FROM antrian_loket WHERE type = 'Apotek' AND postdate LIKE current_date()")); if(empty($q['q'])) { echo '0'; } else { echo $q['q']; } ?></a>
